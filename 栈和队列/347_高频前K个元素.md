...
**思路三**: 上面的思路都是将元素通过排序后进行计数然后再加入到堆中的，其实可以将排序的步骤省略，直接使用 HashMap 来完完成计数这个操作，最后将 EntrySet 加入到一个优先队列中并按照 EntrySet 的 value 进行排序即可，这里需要注意的是在 PriorityQueue 的构造函数的位置通过 lamba 表达式传入了一个比较器，通过这个比较器，内部的堆才能正确建立
```java
class Solution {
    public int[] topKFrequent(int[] nums, int k) {
