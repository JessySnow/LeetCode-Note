...
观察后序遍历的结果，其最后一个元素就代表了这棵树的根节点，我们在中序遍历的结果中找到这个节点，就可以把这个中序遍历的结果分为左右两个部分，这两个部分就是根节点的两棵左右子树，再对这两个子数组进行上面的操作，就能够划分出新的左右子树，只需要一直向下划分，最后就能够找到一个子数组只含有一个元素，这就是叶子节点，图示如下

![恢复二叉树](./picture/recover_binary_tree.png)
